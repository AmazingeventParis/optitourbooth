...
// ── Route maintenance : reset dispatched=false sur tous les points CRM futurs ──
// Usage ponctuel pour corriger les faux positifs de l'auto-dispatch.
// À supprimer après usage.
export async function resetCrmDispatched(req: Request, res: Response): Promise<void> {
  ...
  const candidates = await prisma.pendingPoint.findMany({
    ...
    select: { id: true, externalId: true, clientName: true, type: true, date: true, source: true },
  });
  ...
  apiResponse.success(res, {
    reset: result.count,
    points: candidates.map((p: { externalId: string | null; clientName: string; type: string; date: Date; source: string }) => ({
      externalId: p.externalId,
      clientName: p.clientName,
      type: p.type,
