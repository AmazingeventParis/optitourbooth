...
export default function ReviewPage() {
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      await axios.post(`${API_URL}/public/bookings/${token}/actions/submit-feedback`, {
        feedback: feedbackText.trim(),
      });
    } catch {
      // silent — never block the user on this fake form
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          {/* Low rating (1-3) - Fake Google review + gallery link */}
            ...
            <div className="p-5">
              ...
              <div className="flex justify-center gap-1 mb-3">
                {[1, 2, 3, 4, 5].map((star) => (
                  <svg
                    key={star}
                    className={`w-8 h-8 transition-colors ${star <= selectedRating ? 'text-amber-400' : 'text-gray-200'}`}
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <p className="text-sm text-gray-500 text-center mb-4">
                Nous sommes désolés de ne pas avoir répondu pleinement à vos attentes. Si vous le souhaitez, vous pouvez partager votre expérience en rédigeant un commentaire.
              </p>

              {/* Fake Google review box */}
              {feedbackSubmitted ? (
                <div className="flex items-center gap-2.5 border border-green-200 bg-green-50 rounded-xl px-4 py-3 mb-4">
                  <svg className="h-5 w-5 text-green-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-sm text-gray-600">Merci pour votre commentaire !</span>
                </div>
              ) : (
                <div className="border border-gray-200 rounded-xl shadow-sm overflow-hidden mb-4">
                  {/* Header */}
                  <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-100 bg-white">
                    ...
                    <span className="text-sm font-medium text-gray-700">Ajouter un avis public</span>
                  </div>
                  {/* Body */}
                  <div className="px-4 pt-3 pb-2 bg-white">
                    <div className="flex gap-0.5 mb-2">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <svg key={s} className={`h-5 w-5 ${s <= selectedRating ? 'text-amber-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 24 24">
                          <path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                        </svg>
                      ))}
                    </div>
                    <textarea
                      ...
                      className="w-full text-sm resize-none border-0 focus:outline-none placeholder-gray-300 text-gray-700 leading-relaxed"
                      maxLength={2000}
                    />
                  </div>
                  {/* Footer */}
                  <div className="flex justify-end px-4 py-2 border-t border-gray-100 bg-gray-50">
                    <button
                      ...
                      className="px-5 py-1.5 text-sm font-medium text-white rounded transition-opacity disabled:opacity-60"
          ...
          {pageState === 'ask_review' && (
            <div className="p-6 sm:p-8">
              ...
              <p className="text-gray-600 text-center mb-6">
                En partageant votre expérience, vous soutenez notre travail et aidez de futurs clients à nous découvrir !
              </p>
              ...
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-5 mb-4">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Votre avis compte énormément pour nous ! En remerciement, recevez votre galerie photo par email dans les minutes qui suivent.
                </p>
